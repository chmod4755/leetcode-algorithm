    ...
    class Solution {
        public Node insert(Node head, int insertVal) {
            if (head == null) {
                ...
                Node start = head;
                while (head != start.next) {
                    if (start.next.val < start.val && (insertVal >= start.val || insertVal <= start.next.val)) {
                        //最大值和最小值相交处，如果insert大于最大值或者小于最小值，可插入
                        start.next = new Node(insertVal, start.next);
                        return head;
                    } else if (start.val <= insertVal && insertVal <= start.next.val) {
                        //正常情况，如果insert处在中间，可插入，其实if调教和上面可以合并
                        start.next = new Node(insertVal, start.next);
                        return head;
                    }
                    start = start.next;
                }
                //如果转了一圈了，就地插入吧，没有其他位置了。
                start.next = new Node(insertVal, start.next);
            }
            return head;
        }
